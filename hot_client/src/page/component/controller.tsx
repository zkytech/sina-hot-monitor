 ...
 * @param props 
 */
const Controller:React.FunctionComponent<ControllerProps> =  (props: ControllerProps) => {
    ...
    return (
        <div style = {{padding:'20px'}}>
            {/* 开始/暂停 按钮 */}
            <Button type={'primary'} onClick={()=>displayStatusChange(!paused)}>{paused?"开始":"暂停"}</Button>
            &emsp;&emsp;&emsp;&emsp;
            <span>日期：</span>

            {/* 日期选择器 */}
            <DatePicker defaultValue = {moment(defaultDateTime)} 
            style={{width:'120px'}}
            allowClear = {false}
            ...
            {/* 时间选择器 */}
            <TimePicker defaultValue = {moment(defaultDateTime)}
            style={{width:'120px'}}
            allowClear = {false}
            format = {'HH:mm'}
